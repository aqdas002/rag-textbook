...
describe('RecallPrompt', () => {
    ...
    test('renders question and answer textarea', () => {
        render(_jsx(RecallPrompt, { concept: "c1", question: "Explain X." }));
    ...
    test('submitting answer posts pendingAnswers[concept] to state-writer', async () => {
        ...
        await waitFor(() => {
            expect(fetch).toHaveBeenCalledWith('http://localhost:5174/state', expect.objectContaining({ method: 'POST' }));
        });
        const body = JSON.parse(fetch.mock.calls[0][1].body);
        expect(body.pendingAnswers).toBeDefined();
        expect(body.pendingAnswers.c1).toEqual(expect.objectContaining({ concept: 'c1', answer: 'My answer', question: 'Explain X.' }));
    });
    test('commit clears only its own concept from pendingAnswers / pendingGrades', async () => {
        let subCb;
        stateFile.subscribeToPendingGrade.mockImplementation((cb) => { subCb = cb; return () => { }; });
        render(_jsx(RecallPrompt, { concept: "c1", question: "Q" }));
        await userEvent.type(screen.getByRole('textbox'), 'a');
        await userEvent.click(screen.getByRole('button', { name: /submit/i }));
        subCb({ concept: 'c1', rating: 3, comment: '' });
        fetch.mockClear();
        await userEvent.click(await screen.findByRole('button', { name: /commit to sr/i }));
        const lastCall = fetch.mock.calls.slice(-1)[0];
    ...
    test('when pendingGrade arrives, shows commit-to-SR button', async () => {
        let subCb;
        stateFile.subscribeToPendingGrade.mockImplementation((cb) => {
            subCb = cb;
            return () => { };
        });
        render(_jsx(RecallPrompt, { concept: "c1", question: "Q" }));
