...
const SAMPLE_TEXT = 'The retriever pulls chunks based on similarity. If the answer to your question ' +
...
export function OverlapVisualizer({ initialChunkSize = 80, initialOverlap = 20 }) {
    ...
    return (_jsxs("div", { className: styles.sim, children: [_jsxs("label", { children: ["Chunk size: ", _jsx("strong", { children: chunkSize }), _jsx("input", { type: "range", min: 32, max: 300, step: 8, value: chunkSize, onChange: e => setChunkSize(Number(e.target.value)) })] }), _jsxs("label", { children: ["Overlap: ", _jsx("strong", { children: clampedOverlap }), _jsx("input", { type: "range", min: 0, max: chunkSize - 1, step: 4, value: clampedOverlap, onChange: e => setOverlap(Number(e.target.value)) })] }), _jsxs("p", { className: styles.legend, children: [_jsx("span", { className: styles.overlapSwatch }), " red = shared region (text repeated in adjacent chunks)"] }), _jsx("svg", { ref: svgRef, className: styles.svg })] }));
}
function drawRange(svg, start, end, color, opacity, charsPerLine, lineHeight, width) {
