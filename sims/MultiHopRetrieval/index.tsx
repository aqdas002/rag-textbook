...
export type NodeType = 'company' | 'person' | 'event' | 'product';

export interface GraphNode {
  id: string;
  label: string;
  type: NodeType;
  /** Pre-seeded position [0..1] so layout is stable across renders */
  seedX: number;
  seedY: number;
}
...
// ---------------------------------------------------------------------------
// Fact chunks — one atomic fact per node
// ---------------------------------------------------------------------------

interface FactChunk {
  nodeId: string;
  text: string;
  /** Keywords present in this chunk for toy cosine matching */
  keywords: string[];
}

const FACT_CHUNKS: FactChunk[] = [
...
// ---------------------------------------------------------------------------
// Query definitions — 1 single-hop + 2 multi-hop
// ---------------------------------------------------------------------------

interface TraversalStep {
  nodeId: string;
  edgeLabel?: string; // edge used to arrive here (undefined for start node)
}

interface QueryDef {
  text: string;
  hops: number;
  vanillaAnswered: boolean;
  graphAnswered: boolean;
  /** Ordered traversal path nodes (including start) */
  graphPath: TraversalStep[];
  /** The answer summary shown below GraphRAG column */
  answer: string;
  /** Keywords in the query string, used for toy cosine ranking */
...
function toyCosineSimilarity(queryKeywords: string[], chunk: FactChunk): number {
...
function rankChunksForQuery(queryKeywords: string[]): FactChunk[] {
  return [...FACT_CHUNKS]
...
export function MultiHopRetrieval({ width = 520, height = 420, initialQueryIndex = 0 }: Props) {
  ...
  useEffect(() => {
    ...
    EDGES.forEach(edge => {
      ...
      const isActive =
        activePathEdgeKeys.has(`${edge.source}->${edge.target}`) ||
    ...
    simNodes.forEach(node => {
      ...
      const color = NODE_COLORS[node.type as NodeType];
  ...
  return (
    <div className={styles.sim}>
      <h3 className={styles.title}>Multi-Hop Retrieval: Vanilla RAG vs GraphRAG</h3>

      {/* Query selector */}
      <div className={styles.queryRow}>
        <label htmlFor="query-select" className={styles.queryLabel}>
          <strong>Query:</strong>
        </label>
        <select
          id="query-select"
      ...
      {/* Main layout: graph on left, retrieval panels on right */}
      <div className={styles.mainLayout}>
        {/* LEFT: Knowledge graph SVG */}
        <div className={styles.graphPanel}>
          ...
          {/* Overlay transparent circles with data-testid for stable test selectors */}
          <div className={styles.svgWrapper}>
            ...
            />
            {/* Transparent overlay circles for testability */}
            <svg
              width={width}
              height={height}
              viewBox={`0 0 ${width} ${height}`}
        ...
        {/* RIGHT: Retrieval comparison panels */}
        <div className={styles.retrievalPanel}>
          {/* Vanilla RAG */}
          <div className={styles.retrievalColumn}>
            <h4 className={styles.panelTitle}>
              Vanilla RAG
              <span className={`${styles.badge} ${activeQuery.vanillaAnswered ? styles.badgeGreen : styles.badgeRed}`}>
                {activeQuery.vanillaAnswered ? 'Can answer' : 'Cannot answer'}
              </span>
            </h4>
            <p className={styles.colSubtitle}>Top-5 chunks by cosine similarity</p>
            {vanillaChunks.map((chunk, i) => (
              <div
                key={chunk.nodeId}
                data-testid={`vanilla-chunk-${i}`}
                className={`${styles.chunkCard} ${activePathNodeIds.has(chunk.nodeId) ? styles.chunkRelevant : ''}`}
              >
                <span className={styles.rankBadge}>#{i + 1}</span>
                <span className={styles.chunkText}>{chunk.text}</span>
              </div>
            ))}
            {!activeQuery.vanillaAnswered && (
              <p className={styles.failNote}>
                These chunks each contain one isolated fact. No single chunk chains them together.
              </p>
            )}
          </div>
          ...
          <div className={styles.retrievalColumn}>
            ...
            <p className={styles.colSubtitle}>Traversal path ({activeQuery.hops} hop{activeQuery.hops !== 1 ? 's' : ''})</p>
            <ol className={styles.pathList}>
              {activeQuery.graphPath.map((step, i) => {
                const node = NODES.find(n => n.id === step.nodeId)!;
                return (
                  <li key={step.nodeId} className={styles.pathStep} data-testid={`path-step-${i}`}>
                    <span
                    ...
                    {step.edgeLabel && (
                      <span className={styles.pathEdge}>
                        ← {step.edgeLabel}
                      </span>
                    )}
                  </li>
            ...
            <div className={styles.answerBox}>
              <strong>Answer: </strong>{activeQuery.answer}
            </div>
          </div>
        </div>
      </div>
