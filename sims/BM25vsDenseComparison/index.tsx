...
const DENSE_SYNONYMS = new Map<string, string[]>([
...
function tokeniseBM25(text: string): string[] {
...
function buildBM25Index(corpus: CorpusDoc[]): BM25Index {
...
function bm25Score(
  ...
  const docTF = tf.get(docId) ?? new Map<string, number>();
...
function tokeniseDense(text: string): string[] {
...
function bowCosineExpanded(queryText: string, docText: string): number {
  const qExpanded = expandQueryTokens(tokeniseDense(queryText));
  const dTokens = tokeniseDense(docText);
  if (qExpanded.length === 0 || dTokens.length === 0) return 0;

  const freqQ = new Map<string, number>();
  const freqD = new Map<string, number>();
  for (const t of qExpanded) freqQ.set(t, (freqQ.get(t) ?? 0) + 1);
  for (const t of dTokens) freqD.set(t, (freqD.get(t) ?? 0) + 1);
...
// ---------------------------------------------------------------------------
// RRF (k=60) — only BM25 docs with score > 0 contribute to BM25 ranking.
// This prevents arbitrary ordering of zero-score docs from corrupting the fusion.
// ---------------------------------------------------------------------------
...
function computeRankings(queryText: string): {
  bm25Top: RankedDoc[];
  denseTop: RankedDoc[];
  hybridTop: RankedDoc[];
} {
  ...
  const bm25Top: RankedDoc[] = bm25Scored.map((d, i) => ({ ...d, rank: i + 1 }));
  ...
  const denseTop: RankedDoc[] = denseScored.map((d, i) => ({ ...d, rank: i + 1 }));
...
function computeCrossQueryStats(): {
  ...
  for (const q of QUERIES) {
    ...
    if (bm25Top[0].docId === q.correctDocId) bm25Wins++;
    if (denseTop[0].docId === q.correctDocId) denseWins++;
    if (hybridTop[0].docId === q.correctDocId) hybridWins++;
  }
...
type Winner = 'BM25' | 'Dense' | 'Hybrid' | 'Tie';
...
function determineWinner(
  ...
  if (bm25Rank === minRank) winners.push('BM25');
  if (denseRank === minRank) winners.push('Dense');
  if (hybridRank === minRank) winners.push('Hybrid');
...
export function BM25vsDenseComparison({ initialQueryIndex = 0 }: Props) {
    ...
    <div className={styles.sim}>
      ...
      <div className={styles.columns}>
        ...
        <div className={styles.panel}>
          <h4 className={styles.panelTitle}>BM25 (lexical)</h4>
          {bm25Top.map((doc, i) => {
            const isCorrect = doc.docId === activeQuery.correctDocId;
            return (
              <div
                key={doc.docId}
                data-testid={`bm25-row-${i}`}
                className={`${styles.docCard} ${isCorrect ? styles.correctDoc : ''}`}
              >
        ...
        {/* Dense column */}
        <div className={styles.panel}>
          <h4 className={styles.panelTitle}>Dense (semantic)</h4>
          {denseTop.map((doc, i) => {
            const isCorrect = doc.docId === activeQuery.correctDocId;
            return (
              <div
                key={doc.docId}
                data-testid={`dense-row-${i}`}
                className={`${styles.docCard} ${isCorrect ? styles.correctDoc : ''}`}
              >
                <span className={styles.rankBadge}>#{doc.rank}</span>
                <span className={styles.docText}>
                ...
                <span className={styles.scoreChip}>{doc.score.toFixed(4)}</span>
              </div>
            );
          })}
        </div>
        ...
        {/* Hybrid (RRF) column */}
        <div className={styles.panel}>
          <h4 className={styles.panelTitle}>Hybrid — RRF</h4>
          {hybridTop.map((doc, i) => {
            const isCorrect = doc.docId === activeQuery.correctDocId;
            return (
              <div
                key={doc.docId}
                data-testid={`hybrid-row-${i}`}
                className={`${styles.docCard} ${isCorrect ? styles.correctDoc : ''}`}
              >
                <span className={styles.rankBadge}>#{doc.rank}</span>
                <span className={styles.docText}>
      ...
      {/* Per-query winner badge */}
      <div className={styles.winnerRow}>
        <span
          className={`${styles.winnerBadge} ${styles[`winner${winner}`]}`}
          data-testid="winner-badge"
        >
          {winner} wins
        </span>
        <span className={styles.winnerNote}>
          &nbsp;— correct doc (#{activeQuery.correctDocId}) ranked highest by {winner}
        </span>
      </div>

      {/* Cross-query summary */}
      <div className={styles.summaryPanel}>
        <strong>Across all 5 queries:</strong>
        {' '}Hybrid found the right answer in{' '}
        <strong>{CROSS_QUERY_STATS.hybridWins} of 5</strong> queries;
        {' '}BM25 alone in <strong>{CROSS_QUERY_STATS.bm25Wins}</strong>;
        {' '}Dense alone in <strong>{CROSS_QUERY_STATS.denseWins}</strong>.
      </div>
    </div>
  );
}
