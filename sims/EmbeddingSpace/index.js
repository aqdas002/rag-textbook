...
export function EmbeddingSpace({ width = 700, height = 500 }) {
    ...
    const innerW = width - pad.left - pad.right;
    const innerH = height - pad.top - pad.bottom;
    return (_jsxs("div", { className: styles.sim, children: [_jsx("h3", { className: styles.title, children: "Embedding Space" }), _jsx("p", { className: styles.instructions, children: "Click any word to see its nearest neighbors in the vector space." }), _jsx("div", { className: styles.legend, children: Object.entries(CLUSTER_COLORS).map(([cluster, color]) => (_jsxs("span", { className: styles.legendItem, children: [_jsx("svg", { width: "12", height: "12", children: _jsx("circle", { cx: "6", cy: "6", r: "5", fill: color }) }), cluster] }, cluster))) }), _jsxs("div", { className: styles.svgWrapper, children: [_jsx("svg", { ref: svgRef, width: width, height: height, viewBox: `0 0 ${width} ${height}`, className: styles.svg }), _jsx("svg", { width: width, height: height, viewBox: `0 0 ${width} ${height}`, className: styles.svgOverlay, style: { position: 'absolute', top: 0, left: 0 }, children: WORDS.map(wordData => {
                            const xScale = d3.scaleLinear().domain([0, 1]).range([0, innerW]);
                            const yScale = d3.scaleLinear().domain([0, 1]).range([innerH, 0]);
                            const cx = xScale(wordData.x) + pad.left;
                            const cy = yScale(wordData.y) + pad.top;
                            return (_jsx("circle", { "data-testid": `word-dot-${wordData.word}`, cx: cx, cy: cy, r: 12, fill: "transparent", style: { cursor: 'pointer' }, onClick: () => setClickedWord(wordData.word === clickedWord ? null : wordData.word) }, wordData.word));
                        }) })] }), clickedWord && (_jsxs("div", { className: styles.panel, children: [_jsx("strong", { children: clickedWord }), " \u2014 top 5 nearest neighbors", _jsx("ul", { className: styles.neighborList, children: neighbors.map(n => (_jsxs("li", { children: [_jsx("span", { className: styles.neighborDot, style: { background: CLUSTER_COLORS[n.cluster] ?? '#6b7280' } }), _jsx("span", { className: styles.neighborWord, children: n.word }), _jsx("span", { className: styles.neighborScore, children: n.score.toFixed(3) })] }, n.word))) }), _jsxs("p", { className: styles.purity, children: ["Cluster purity: ", _jsxs("strong", { children: [(clusterPurity * 100).toFixed(0), "%"] }), ' ', "(", neighbors.filter(n => n.cluster === clickedCluster).length, " of 5 neighbors are in the ", _jsx("em", { children: clickedCluster }), " cluster)"] })] }))] }));
}
