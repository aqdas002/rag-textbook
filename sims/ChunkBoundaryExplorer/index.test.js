...
describe('ChunkBoundaryExplorer', () => {
    test('renders text and chunk-size slider', () => {
        render(_jsx(ChunkBoundaryExplorer, { initialChunkSize: 128 }));
    ...
    test('changing slider reports state via reportState', async () => {
        ...
        expect(reportStateModule.reportState).toHaveBeenCalledWith('ChunkBoundaryExplorer', expect.objectContaining({ chunkSize: 256 }));
    });
    test('chunk count = ceil(text_length / chunk_size)', () => {
        render(_jsx(ChunkBoundaryExplorer, { initialChunkSize: 100, textLength: 350 }));
